    ...
    try {
        ...
        headlines = data.results.map(result => {
            ...
            if (result.multimedia && result.multimedia.length) {
                // Change to 'threeByTwoSmallAt2X' format
                const image = result.multimedia.find(media => media.format === 'threeByTwoSmallAt2X');
                imageUrl = image ? image.url : null;
            }
            return { title: result.title, imageUrl: imageUrl };
        });

        // Preload all images after fetching headlines
        const imageUrls = headlines.map(headline => headline.imageUrl).filter(url => url != null);
...
function displayNextHeadline() {
    ...
    const headlineImageElement = document.getElementById("headlineImage");
    const currentHeadline = headlines[currentHeadlineIndex];

    // Display the image
    if (currentHeadline.imageUrl) {
        headlineImageElement.src = currentHeadline.imageUrl;
        headlineImageElement.style.display = 'block';
        headlineElement.style.display = 'none';

        // After 1 second, hide the image and show the headline text
        setTimeout(() => {
